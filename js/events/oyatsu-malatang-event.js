import { suspendAudio, resumeAudio, playSfx, vibrate } from '../audio.js?v=0.10.951';
...
let running=false,stage='intro1',overlay=null,video=null,bgm=null,skipTimer=0,finishing=false,charged=false,capturedButton=null;
...
function ensureStyle(){
  ...
  const l=document.createElement('link');l.id=STYLE_ID;l.rel='stylesheet';l.href='./oyatsu-malatang-event.css?v=0.10.951';document.head.appendChild(l);
}
function playBgm(){
  try{
...
function renderDialogue(){
  ...
  <main class="main-screen malatang-dialogue-screen"><section class="visit-character-event">
    <div class="visit-character-area"><img class="visit-character jxj-new-event-character" src="${CHARACTER}" alt="おやつ大好き" draggable="false"></div>
...
function settle(){
  ...
  try{
    ...
    if(!r?.ok)return false;charged=true;return true;
...
function lockMeal(){
  ...
  p.style.cssText='display:grid!important;grid-template-rows:minmax(0,1fr) 38px!important;overflow:hidden!important;padding:10px 16px 12px!important';
  f.style.cssText='width:100%!important;height:100%!important;min-height:0!important;margin:0!important;padding:10px 12px 14px!important;display:grid!important;place-items:center!important;overflow:hidden!important;box-sizing:border-box!important';
  const ih=Math.max(120,Math.min(Math.round(innerHeight*.32),190));
  i.style.cssText=`display:block!important;width:auto!important;height:${ih}px!important;max-height:${ih}px!important;max-width:min(38vw,430px)!important;margin:0 auto!important;object-fit:contain!important;object-position:center center!important`;
  c.style.cssText='position:static!important;height:38px!important;min-height:38px!important;margin:0!important;padding:0!important;display:grid!important;place-items:center!important';
}
function renderMeal(){
  if(!settle())return cleanup(false,true);
  stage='meal';patch({active:true,stage,charged:true});
  ...
  requestAnimationFrame(lockMeal);playSfx('eat',{gain:1});vibrate([55,45,55,45,70]);
}
function cleanMovie(){
...
function cleanup(completed=false,resumeNormal=false){
  if(!running)return;running=false;cleanMovie();stopBgm();overlay?.remove();overlay=null;patch({active:false,stage:'idle',charged:false,completed:Boolean(completed)});charged=false;finishing=false;resumeAudio().catch(()=>{});
...
function onOverlayClick(e){
  const b=e.target instanceof Element?e.target.closest('[data-mala]'):null;if(!b||!overlay?.contains(b))return;playBgm();playSfx('select',{gain:.72});
  const a=b.dataset.mala;if(a==='end')cleanup(false,false);else if(a==='movie-skip')finishMovie();else if(a==='meal-finish'&&stage==='meal'){persist('after1');renderDialogue()}else if(a==='next')advance();
}
