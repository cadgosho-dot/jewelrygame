...
assert(VERSION === '0.10.488', `VERSIONが0.10.488ではありません: ${VERSION}`);
assert(DEFAULT_BIRTHDAY === '04-01', '誕生日の既定値が4月1日ではありません');
assert(sw.includes("const VERSION = '0.10.488'"), 'Service WorkerのVERSIONが古いです');
assert(indexHtml.includes('game.html?v=0.10.488'), 'index.htmlの読込番号が古いです');
assert(gameHtml.includes('js/app.js?v=0.10.488'), 'game.htmlの読込番号が古いです');
assert(app.includes("./daily-gems.js?v=0.10.488"), 'daily-gemsの読込番号が古いです');
const stylesV488 = fs.readFileSync(path.join(root, 'styles.css'), 'utf8');
assert(stylesV488.includes('v0.10.485: 縦画面の上部バー1を大型文字・縦中央・低余白へ調整'), '上部バー1のv0.10.488スタイルがありません');
assert(stylesV488.includes('--jwj-two-bar-info-height:clamp(46px'), '上部バー1の高さ圧縮指定がありません');
assert(stylesV488.includes('font-size:clamp(12px,3.55vw,14.5px)!important'), '上部バー1の文字拡大指定がありません');
assert(stylesV488.includes('justify-content:center!important'), '上部バー1の縦中央指定がありません');
assert(stylesV488.includes('v0.10.486: 原石研磨完成画面の石と結果枠をさらに縮小'), '原石研磨完了画面のv0.10.488スタイルがありません');
assert(stylesV488.includes('width:min(330px,calc(100vw - 36px))'), '通常画面の結果枠縮小指定がありません');
assert(stylesV488.includes('width:min(286px,calc(100vw - 40px))'), '携帯縦画面の結果枠縮小指定がありません');
assert(stylesV488.includes('width:min(112px,28vw,18dvh)'), '通常画面の完成ルース縮小指定がありません');
assert(stylesV488.includes('width:min(88px,28vw,16dvh)'), '携帯縦画面の完成ルース縮小指定がありません');
assert(stylesV488.includes('width:min(92px,18vw,34dvh)'), '低い横画面の完成ルース縮小指定がありません');

assert(stylesV488.includes('v0.10.488 全イベント共通の緊急終了導線：画面上端・左端へ控えめに配置'), 'イベント終了ボタンのv0.10.488スタイルがありません');
assert(stylesV488.includes('top:env(safe-area-inset-top,0px)'), 'イベント終了ボタンが画面上端に配置されていません');
assert(stylesV488.includes('left:env(safe-area-inset-left,0px)'), 'イベント終了ボタンが左詰めではありません');
assert(stylesV488.includes('right:auto'), 'イベント終了ボタンの右寄せ指定が残っています');
assert(stylesV488.includes('min-height:17px'), '携帯用イベント終了ボタンが十分小さくありません');
assert(stylesV488.includes('font-size:7.5px'), '携帯用イベント終了ボタンの文字が小型化されていません');
assert(stylesV488.includes('opacity:.46'), 'イベント終了ボタンが控えめな表示ではありません');

const kaitenzushiHtmlV487 = fs.readFileSync(path.join(root, 'assets/minigames/kaitenzushi/game/index.html'), 'utf8');
assert(app.includes('function kaitenzushiFrameSource()'), '回転寿司のバージョン付きiframe URL生成がありません');
assert(app.includes("dataset?.jxjKaitenzushi === '1'"), '回転寿司文書の識別確認がありません');
assert(app.includes("message.type === 'ready'"), '回転寿司の起動完了通知処理がありません');
assert(app.includes("data-action=\"retry-kaitenzushi\""), '回転寿司の再読込操作がありません');
assert(app.includes("data-action=\"cancel-kaitenzushi\""), '回転寿司から食事選択へ戻る操作がありません');
assert(sw.includes('async function kaitenzushiDocumentNetworkFirst(request)'), '回転寿司専用のService Worker取得処理がありません');
assert(sw.indexOf("url.pathname.endsWith('/assets/minigames/kaitenzushi/game/index.html')") < sw.indexOf("event.request.mode === 'navigate'"), '回転寿司専用ルートが共通文書ルートより後です');
assert(kaitenzushiHtmlV487.includes('data-jxj-kaitenzushi="1"'), '回転寿司文書の識別マーカーがありません');
assert(kaitenzushiHtmlV487.includes('sendIntegrationMessage("ready")'), '回転寿司の起動完了通知がありません');
...
// v0.10.488: 12月23日〜1月3日は体調不良イベントを発生させない。
{
  const context = { Date };
  vm.createContext(context);
...
console.log('v0.10.488 年末年始の体調不良停止期間検査: OK');
...
// v0.10.488: user-provided tool image implementation.
const toolImagesV486 = [
...
const gameDataTextV486 = fs.readFileSync(path.join(root, 'js/game-data.js'), 'utf8');
for (const filename of toolImagesV486) {
  ...
  assert(gameDataTextV486.includes(`./assets/images/tools/${filename}`), `データ参照がありません: ${filename}`);
}
console.log(`v0.10.488 validation passed: ${toolImagesV486.length} tool images implemented including buffer.png.`);
