...
assert(VERSION === '0.10.492', `VERSIONが0.10.492ではありません: ${VERSION}`);
assert(DEFAULT_BIRTHDAY === '04-01', '誕生日の既定値が4月1日ではありません');
assert(sw.includes("const VERSION = '0.10.492'"), 'Service WorkerのVERSIONが古いです');
assert(indexHtml.includes('game.html?v=0.10.492'), 'index.htmlの読込番号が古いです');
assert(gameHtml.includes('js/app.js?v=0.10.492'), 'game.htmlの読込番号が古いです');
assert(app.includes("./daily-gems.js?v=0.10.492"), 'daily-gemsの読込番号が古いです');
...
// v0.10.492: meal content must begin below the visible header.
assert(app.includes("contentClass: 'meal-eating-screen-content'"), '食事中画面に専用コンテンツクラスがありません');
assert(app.includes("const contentClass = ['screen-content'"), 'shellが画面固有コンテンツクラスを受け取れません');
assert(app.includes("const mealAnchor = screen === 'meal' ? root.querySelector('.meal-eating-panel') : null;"), '食事枠上端の実測補正がありません');
assert(app.includes("(!portrait && screen !== 'meal')"), '横画面の食事画面で上部バー補正が実行されません');
assert(stylesV489.includes('v0.10.492: 食事中の料理枠を上部バーの下へ固定し、上端の隠れを防止'), '食事画面の上部バー重なり防止CSSがありません');
assert(stylesV489.includes('body[data-screen="meal"] .meal-eating-screen-content'), '食事中専用コンテンツ配置がありません');
assert(stylesV489.includes('justify-content:flex-start!important'), '食事枠が上端から安全に配置されません');

const kaitenzushiHtmlV490 = fs.readFileSync(path.join(root, 'assets/minigames/kaitenzushi/game/index.html'), 'utf8');
assert(app.includes("from './kaitenzushi-embedded.js?v=0.10.492'"), '回転寿司の埋め込み文書モジュールを読み込んでいません');
...
assert(sw.includes("'./js/kaitenzushi-embedded.js?v=0.10.492'"), 'Service Workerのコアキャッシュに埋め込み文書モジュールがありません');
...
console.log('v0.10.492 年末年始の体調不良停止期間検査: OK');
...
// v0.10.492: all 19 user-provided tool images must be physically present and packaged.
const toolImagesV492 = [
...
const gameDataTextV492 = fs.readFileSync(path.join(root, 'js/game-data.js'), 'utf8');
for (const filename of toolImagesV492) {
  ...
  assert(gameDataTextV492.includes(`./assets/images/tools/${filename}`), `データ参照がありません: ${filename}`);
}
console.log(`v0.10.492 validation passed: ${toolImagesV492.length} tool images implemented including buffer.png.`);
