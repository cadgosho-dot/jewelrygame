...
start = app.index('function syncScreenContentTopOffset() {')
end = app.index('\nfunction scheduleScreenContentTopOffsetSync()', start)
...
with sync_playwright() as p:
    ...
    for width, height in viewports:
        ...
        for index, screen_id in enumerate(screens):
            ...
            if result['headerBottom'] > 0.5 and abs(result['gap'] - 8) > 0.51:
...
print(f'縦画面ヘッダー検査: OK（{len(screens)}画面 × {len(viewports)}端末サイズ = {len(screens)*len(viewports)}ケース）')
print('上部バーと本文先頭の間隔は、旧固定余白が存在する状態でも全ケース8pxです。')
