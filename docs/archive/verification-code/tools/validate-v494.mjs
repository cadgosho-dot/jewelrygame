...
assert(VERSION === '0.10.494', `VERSIONが0.10.494ではありません: ${VERSION}`);
assert(DEFAULT_BIRTHDAY === '04-01', '誕生日の既定値が4月1日ではありません');
assert(sw.includes("const VERSION = '0.10.494'"), 'Service WorkerのVERSIONが古いです');
assert(indexHtml.includes('game.html?v=0.10.494'), 'index.htmlの読込番号が古いです');
assert(gameHtml.includes('js/app.js?v=0.10.494'), 'game.htmlの読込番号が古いです');
assert(app.includes("./daily-gems.js?v=0.10.494"), 'daily-gemsの読込番号が古いです');
...
assert(app.includes("from './kaitenzushi-embedded.js?v=0.10.494'"), '回転寿司の埋め込み文書モジュールを読み込んでいません');
...
assert(sw.includes("'./js/kaitenzushi-embedded.js?v=0.10.494'"), 'Service Workerのコアキャッシュに埋め込み文書モジュールがありません');
...
console.log('v0.10.494 年末年始の体調不良停止期間検査: OK');
...
console.log(`v0.10.494 validation passed: ${toolImagesV493.length} tool images implemented including buffer.png.`);
