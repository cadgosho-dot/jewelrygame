...
assert(VERSION === '0.10.487', `VERSIONが0.10.487ではありません: ${VERSION}`);
assert(DEFAULT_BIRTHDAY === '04-01', '誕生日の既定値が4月1日ではありません');
assert(sw.includes("const VERSION = '0.10.487'"), 'Service WorkerのVERSIONが古いです');
assert(indexHtml.includes('game.html?v=0.10.487'), 'index.htmlの読込番号が古いです');
assert(gameHtml.includes('js/app.js?v=0.10.487'), 'game.htmlの読込番号が古いです');
assert(app.includes("./daily-gems.js?v=0.10.487"), 'daily-gemsの読込番号が古いです');
const stylesV486 = fs.readFileSync(path.join(root, 'styles.css'), 'utf8');
assert(stylesV486.includes('v0.10.485: 縦画面の上部バー1を大型文字・縦中央・低余白へ調整'), '上部バー1のv0.10.487スタイルがありません');
assert(stylesV486.includes('--jwj-two-bar-info-height:clamp(46px'), '上部バー1の高さ圧縮指定がありません');
assert(stylesV486.includes('font-size:clamp(12px,3.55vw,14.5px)!important'), '上部バー1の文字拡大指定がありません');
assert(stylesV486.includes('justify-content:center!important'), '上部バー1の縦中央指定がありません');
assert(stylesV486.includes('v0.10.486: 原石研磨完成画面の石と結果枠をさらに縮小'), '原石研磨完了画面のv0.10.487スタイルがありません');
assert(stylesV486.includes('width:min(330px,calc(100vw - 36px))'), '通常画面の結果枠縮小指定がありません');
assert(stylesV486.includes('width:min(286px,calc(100vw - 40px))'), '携帯縦画面の結果枠縮小指定がありません');
assert(stylesV486.includes('width:min(112px,28vw,18dvh)'), '通常画面の完成ルース縮小指定がありません');
assert(stylesV486.includes('width:min(88px,28vw,16dvh)'), '携帯縦画面の完成ルース縮小指定がありません');
assert(stylesV486.includes('width:min(92px,18vw,34dvh)'), '低い横画面の完成ルース縮小指定がありません');
...
// v0.10.487: 12月23日〜1月3日は体調不良イベントを発生させない。
{
  const context = { Date };
  vm.createContext(context);
...
console.log('v0.10.487 年末年始の体調不良停止期間検査: OK');
...
// v0.10.487: user-provided tool image implementation.
...
console.log(`v0.10.487 validation passed: ${toolImagesV486.length} tool images implemented including buffer.png.`);
