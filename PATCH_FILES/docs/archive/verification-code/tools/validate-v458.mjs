...
assert(VERSION === '0.10.458', 'VERSIONが0.10.458ではありません');
assert(sw.includes("const VERSION = '0.10.458'"), 'Service Workerが0.10.458ではありません');
...
const viewportShell = fs.readFileSync(path.join(root, 'viewport-shell.js'), 'utf8');
assert(viewportShell.includes("serviceWorker.register('./sw.js')"), '外側シェルのService Worker復旧登録がありません');
assert(sw.includes('const CORE_SHELL = ['), 'Service Workerの必須起動ファイル定義がありません');
assert(sw.includes('Promise.allSettled(supplemental.map'), '補助キャッシュ失敗の分離処理がありません');
for (const missingTool of ['buffer.png','dividers.png','engraving-block.png','file.png','hammer.png','magnifier.png','milgrain-tool.png','rolling-mill.png','stamps.png','wood-block.png']) {
  assert(!sw.includes(`./assets/images/tools/${missingTool}`), `存在しない工具画像が必須キャッシュに残っています: ${missingTool}`);
}

console.log('v0.10.458 専用検査: OK');
