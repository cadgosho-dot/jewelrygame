...
assert(VERSION === '0.10.497', `VERSIONが0.10.497ではありません: ${VERSION}`);
assert(DEFAULT_BIRTHDAY === '04-01', '誕生日の既定値が4月1日ではありません');
assert(sw.includes("const VERSION = '0.10.497'"), 'Service WorkerのVERSIONが古いです');
assert(indexHtml.includes('game.html?v=0.10.497'), 'index.htmlの読込番号が古いです');
assert(gameHtml.includes('js/app.js?v=0.10.497'), 'game.htmlの読込番号が古いです');
assert(app.includes("./daily-gems.js?v=0.10.497"), 'daily-gemsの読込番号が古いです');
...
assert(app.includes("from './kaitenzushi-embedded.js?v=0.10.497'"), '回転寿司の埋め込み文書モジュールを読み込んでいません');
...
assert(sw.includes("'./js/kaitenzushi-embedded.js?v=0.10.497'"), 'Service Workerのコアキャッシュに埋め込み文書モジュールがありません');
...
// v0.10.497: hunger warning remains visible but is no longer a shortcut button.
assert(!app.includes('タップして食事へ'), '空腹時の「タップして食事へ」が残っています');
assert(!app.includes('hunger-meal-shortcut'), '空腹時の食事ショートカットクラスが残っています');
assert(app.includes('<div class="hunger-lock-notice" role="status" aria-live="polite">'), '空腹警告が非操作メッセージになっていません');
assert(app.includes('<strong>空腹で動けません</strong><span>食事をするか、今日は休んでください。</span></div>'), '空腹時の警告文が残っていません');
assert(app.includes('data-screen="meal" ${mealDisabled}'), '通常の食事メニューボタンがありません');
assert(!stylesV489.includes('.hunger-meal-shortcut{'), '空腹時ショートカット用CSSが残っています');
assert(stylesV489.includes('v0.10.497: 空腹時の案内を非操作のメッセージ表示へ変更'), 'v0.10.497の空腹警告CSSがありません');

// v0.10.496: daily result is grouped and meal remains the final information section.
const renderDayResultV496 = extractFunction(app, 'renderDayResult');
const resultHeadingsV496 = ['今日の作業', '販売・店舗', 'スタッフ', '今日の収支', '食事'];
let previousHeadingIndexV496 = -1;
for (const heading of resultHeadingsV496) {
  const headingIndex = renderDayResultV496.indexOf(`>${heading}<`);
  assert(headingIndex > previousHeadingIndexV496, `1日の結果の区分順が不正です: ${heading}`);
  previousHeadingIndexV496 = headingIndex;
}
assert(renderDayResultV496.includes("activityRows.length ?"), '実績がない作業を「なし」にまとめる処理がありません');
assert(renderDayResultV496.includes('if (mined.length)'), '採掘実績だけを表示する条件がありません');
assert(renderDayResultV496.includes('if (polished.length)'), '研磨実績だけを表示する条件がありません');
assert(renderDayResultV496.includes('if (crafted.length)'), '制作実績だけを表示する条件がありません');
assert(renderDayResultV496.includes('if (roughSoldCount > 0)'), '原石売却実績だけを表示する条件がありません');
assert(renderDayResultV496.includes('if (looseSoldCount > 0)'), 'ルース売却実績だけを表示する条件がありません');
assert(renderDayResultV496.includes('if (sold.length)'), 'ジュエリー販売実績だけを表示する条件がありません');
assert(renderDayResultV496.includes('class="day-result-balance"'), '収支表示がありません');
assert(renderDayResultV496.includes('const balance = income - expense'), '収支が売上－支出で計算されていません');
assert(renderDayResultV496.indexOf('day-result-meal-section') > renderDayResultV496.indexOf('day-result-finance-heading'), '食事が収支より下にありません');
assert(!renderDayResultV496.includes('casesUsed'), '1日の結果にケース使用数が表示されています');
assert(!renderDayResultV496.includes('casesRemaining'), '1日の結果にケース残数が表示されています');
assert(stylesV489.includes('v0.10.496: 1日の結果を作業・販売・スタッフ・収支・食事へ整理'), '結果画面整理用CSSがありません');
assert(stylesV489.includes('.day-result-sections{'), '結果区分のコンテナCSSがありません');
assert(stylesV489.includes('.day-result-meal-section{'), '食事最下部区分のCSSがありません');
...
console.log('v0.10.497 年末年始の体調不良停止期間検査: OK');
...
console.log(`v0.10.497 validation passed: ${toolImagesV493.length} tool images implemented including buffer.png.`);
