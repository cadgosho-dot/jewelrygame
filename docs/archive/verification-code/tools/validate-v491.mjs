...
assert(VERSION === '0.10.491', `VERSIONが0.10.491ではありません: ${VERSION}`);
assert(DEFAULT_BIRTHDAY === '04-01', '誕生日の既定値が4月1日ではありません');
assert(sw.includes("const VERSION = '0.10.491'"), 'Service WorkerのVERSIONが古いです');
assert(indexHtml.includes('game.html?v=0.10.491'), 'index.htmlの読込番号が古いです');
assert(gameHtml.includes('js/app.js?v=0.10.491'), 'game.htmlの読込番号が古いです');
assert(app.includes("./daily-gems.js?v=0.10.491"), 'daily-gemsの読込番号が古いです');
...
assert(app.includes("from './kaitenzushi-embedded.js?v=0.10.491'"), '回転寿司の埋め込み文書モジュールを読み込んでいません');
...
assert(sw.includes("'./js/kaitenzushi-embedded.js?v=0.10.491'"), 'Service Workerのコアキャッシュに埋め込み文書モジュールがありません');
...
console.log('v0.10.491 年末年始の体調不良停止期間検査: OK');
...
// v0.10.491: all 19 user-provided tool images must be physically present and packaged.
const toolImagesV491 = [
...
const gameDataTextV491 = fs.readFileSync(path.join(root, 'js/game-data.js'), 'utf8');
for (const filename of toolImagesV491) {
  ...
  assert(gameDataTextV491.includes(`./assets/images/tools/${filename}`), `データ参照がありません: ${filename}`);
}
console.log(`v0.10.491 validation passed: ${toolImagesV491.length} tool images implemented including buffer.png.`);
