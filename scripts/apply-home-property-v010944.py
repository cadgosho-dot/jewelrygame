...
NEW_VERSION = '0.10.944'

if OLD_VERSION != '0.10.943':
    raise SystemExit(f'FAIL: expected VERSION 0.10.943, got {OLD_VERSION}')
...
store_import = f"import {{ calculateStoreMonthlyRent }} from './finance/store-rent.js?v={OLD_VERSION}';\n"
home_import = f"""import {{ calculateStoreMonthlyRent }} from './finance/store-rent.js?v={OLD_VERSION}';
import {{
  HOME_MOVE_COST,
...
}} from './finance/home-property.js?v={OLD_VERSION}';
"""
...
replace_once(
    ...
    "  const asset = backgroundAssetFor(screen);\n  const backgroundFile = backgroundFor(screen) === 'sleep' && currentHomePropertyId() === 'B'\n    ? `${asset}.png`\n    : `${asset}.webp`;\n  document.body.dataset.backgroundLayout = backgroundLayoutFor(screen, asset);\n  document.documentElement.style.setProperty('--screen-bg', `url('./assets/images/${backgroundFile}?v=${VERSION}')`);\n",
    'sleep background file extension',
)
...
replace_once(
    sw,
    f"'./js/app.js?v={OLD_VERSION}', './js/finance/store-rent.js?v={OLD_VERSION}',",
    f"'./js/app.js?v={OLD_VERSION}', './js/finance/store-rent.js?v={OLD_VERSION}', './js/finance/home-property.js?v={OLD_VERSION}',",
    'service worker home property module',
)

subprocess.run([sys.executable, 'scripts/version-sync.py', '--set', NEW_VERSION], cwd=ROOT, check=True)
...
entry = '''## v0.10.944
- 351日目から不動産屋に「自宅」を追加し、現在の物件Aと新しい物件Ｂを比較して相互に引越せるようにした。現在住んでいる物件は見た目で判別でき、その物件には「引越す」ボタンを表示しない。
- 引越し時は1,000,000円と移動先1ヶ月分の家賃を一括で支払う。物件Aは既存家賃70,000円を維持し、物件Ｂは月額200,000円。通常の毎月15日の自宅家賃も現在の物件に応じて計算する。
- 物件Aは既存の「寝る」背景を維持し、物件Ｂはユーザー提供の横・縦PNG背景を使用する。引越した当日の「寝る」から選択中物件の背景へ切り替える。
- 自宅物件の解放日・料金・背景選択ロジックは `js/finance/home-property.js` に分離し、既存セーブは物件Aとして互換維持する。通常イベントUI・クイズイベントUI・Firebase/クラウド保存方式は変更しない。
