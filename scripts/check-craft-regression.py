...
checks = {
    ...
    'existing jewelry history retained as migration fallback': '__JXJ_MEMORIES_STATE__' in LAST_METAL and 'snapshot?.inventory?.jewelry' in LAST_METAL,
    'completed metal preference helper exported': 'export function rememberCompletedCraftMetal(' in LAST_METAL,
    'completed metal preference is read before history fallback': 'storedCraftMetal(storage) || lastPlayerCraftMetal(jewelry)' in LAST_METAL,
    'metal choice is tracked without app.js expansion': "data-group=\"metal\"" in LAST_METAL and 'handleCraftChoiceClick' in LAST_METAL,
    'metal is persisted only when craft reaches completion': "screen === 'completion'" in LAST_METAL and 'rememberCompletedCraftMetal(browserStorage(), craftSessionMetal)' in LAST_METAL,
    ...
    'non-player jewelry does not replace migration history': all(marker in LAST_METAL for marker in ('workshopStaff', 'autopilot', 'jewelryShop', 'receivedGiftCode')),
    'game save schema is not written by last metal module': all(marker not in LAST_METAL for marker in ('saveGame(', 'indexedDB', 'firebase')),
    'last metal regression covers stored preference': 'rememberCompletedCraftMetal' in LAST_METAL_TEST and 'previousCraftMetal' in LAST_METAL_TEST and 'LAST_CRAFT_METAL_STORAGE_KEY' in LAST_METAL_TEST,
