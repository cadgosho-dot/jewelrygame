...
assert(VERSION === '0.10.486', `VERSIONが0.10.486ではありません: ${VERSION}`);
assert(DEFAULT_BIRTHDAY === '04-01', '誕生日の既定値が4月1日ではありません');
assert(sw.includes("const VERSION = '0.10.486'"), 'Service WorkerのVERSIONが古いです');
assert(indexHtml.includes('game.html?v=0.10.486'), 'index.htmlの読込番号が古いです');
assert(gameHtml.includes('js/app.js?v=0.10.486'), 'game.htmlの読込番号が古いです');
assert(app.includes("./daily-gems.js?v=0.10.486"), 'daily-gemsの読込番号が古いです');
const stylesV486 = fs.readFileSync(path.join(root, 'styles.css'), 'utf8');
assert(stylesV486.includes('v0.10.485: 縦画面の上部バー1を大型文字・縦中央・低余白へ調整'), '上部バー1のv0.10.486スタイルがありません');
...
assert(stylesV486.includes('v0.10.486: 原石研磨完成画面の石と結果枠をさらに縮小'), '原石研磨完了画面のv0.10.486スタイルがありません');
...
// v0.10.486: 12月23日〜1月3日は体調不良イベントを発生させない。
{
  const context = { Date };
  vm.createContext(context);
...
console.log('v0.10.486 年末年始の体調不良停止期間検査: OK');
...
// v0.10.486: user-provided tool image implementation.
...
console.log(`v0.10.486 validation passed: ${toolImagesV486.length} tool images implemented including buffer.png.`);
