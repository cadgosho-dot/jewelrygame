...
const checks = [
  ...
  ['初日当10,000円', data.includes('dailyWage: 10000')],
  ['日当段階上昇', data.includes('dailyWage: 15000') && data.includes('dailyWage: 22000') && data.includes('dailyWage: 32000') && data.includes('dailyWage: 45000')],
  ...
  ['日当支払い', app.includes('payFixedCost(`職人スタッフの日当（Lv.${beforeDefinition.level}）`, wage') && app.includes('paidWage: wageResult.paid')],
  ['給与未払いを保持', app.includes('staff.wageUnpaid = Math.max(0, Number(staff.wageUnpaid) || 0) + unpaid')],
  ['一日の結果', app.includes('<div><span>職人スタッフ</span>') && app.includes('自動制作${result.workshopStaff.crafted}点')],
  ['セーブ互換', data.includes('workshopStaffSource') && data.includes('workMinutesBank') && data.includes('workedMinutesToday') && data.includes('wageUnpaid')],
