...
boot_function_start = APP.find('async function boot() {')
boot_function = APP[boot_function_start:] if boot_function_start >= 0 else ''
boot_start = boot_function.find('indexedDbSave = await readIndexedDbSave(user.uid);')
claim_start = boot_function.find('claimSession(user.uid, sessionId)')
cloud_start = boot_function.find('loadState(user.uid)')
...
checks = {
    ...
    '起動時にクラウドより先にIndexedDBを読む': 0 <= boot_start < claim_start < cloud_start,
