...
assert(VERSION === '0.10.484', `VERSIONが0.10.484ではありません: ${VERSION}`);
assert(DEFAULT_BIRTHDAY === '04-01', '誕生日の既定値が4月1日ではありません');
assert(sw.includes("const VERSION = '0.10.484'"), 'Service WorkerのVERSIONが古いです');
assert(indexHtml.includes('game.html?v=0.10.484'), 'index.htmlの読込番号が古いです');
assert(gameHtml.includes('js/app.js?v=0.10.484'), 'game.htmlの読込番号が古いです');
assert(app.includes("./daily-gems.js?v=0.10.484"), 'daily-gemsの読込番号が古いです');
...
// v0.10.484: 12月23日〜1月3日は体調不良イベントを発生させない。
{
  const context = { Date };
  vm.createContext(context);
...
console.log('v0.10.484 年末年始の体調不良停止期間検査: OK');
...
// v0.10.484: user-provided tool image implementation.
const toolImagesV484 = [
...
const gameDataTextV484 = fs.readFileSync(path.join(root, 'js/game-data.js'), 'utf8');
for (const filename of toolImagesV484) {
  ...
  assert(gameDataTextV484.includes(`./assets/images/tools/${filename}`), `データ参照がありません: ${filename}`);
}
console.log(`v0.10.484 validation passed: ${toolImagesV484.length} tool images implemented including buffer.png.`);
