...
// v0.10.753以降は大容量の基礎データをgame-data-core.jsへ分離している。
// 現行仕様の静的検査ではwrapperとcoreの両方を対象にする。
const data = [
  fs.readFileSync(path.join(root, 'js/game-data.js'), 'utf8'),
  fs.readFileSync(path.join(root, 'js/game-data-core.js'), 'utf8'),
].join('\n');
...
const checks = [
  ...
  ['初期能力が低い', data.includes("level: 1, label: '見習い', minWorkDays: 0, dailyWage: 5000, visitorBonus: 0, customerVisitBonus: 0.01, purchaseBonus: 0.01, saleBonus: 0.02")],
  ['来店人数効果', app.includes('branchVisitors += 2 + storeStaffVisitorBonus(activeEmployee)')],
  ['来店抽選効果', app.includes('storeStaffCustomerVisitBonus(visitEmployee)')],
  ['購入率効果', app.includes('storeStaffPurchaseBonus(branchNumber)')],
  ['販売率効果', app.includes('clamp(0.16 + storeStaffSaleBonus(activeEmployee), 0.16, 0.30)')],
  ['成長後給与を使用', app.includes('const salary = Math.max(0, Math.floor(Number(beforeDefinition.dailyWage) || 5000))')],
  ['給与未払いを保持', app.includes('employee.wageUnpaid = Math.max(0, Number(employee.wageUnpaid) || 0) + unpaid')],
