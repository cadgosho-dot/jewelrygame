...
import {
  ROBOT_STORE_ASSESSMENT_EVENT_KEY,
  ROBOT_STORE_ASSESSMENT_TRIGGER_CHANCE,
  buildRobotStoreAssessmentDialogue,
} from './robot-store-assessment-rules.js?v=0.10.964';

const OVERLAY_ID = 'jxj-robot-store-assessment-overlay';
const STYLE_ID = 'jxj-robot-store-assessment-style';
const CHARACTER_IMAGE = './assets/images/events/store-assessment-robot.png';
const INTERCEPT_KEY = '__JXJ_I';
const RESUME_KEY = '__JXJ_R';
const OWN_RESUME_KEY = '__JXJ_ROBOT_STORE_ASSESSMENT_RESUME__';
...
function patchEvent(patch) {
  try { return helpers()?.patchEventState?.(ROBOT_STORE_ASSESSMENT_EVENT_KEY, patch) || null; } catch (_) { return null; }
}
...
function eventState(stateSnapshot = snapshot()) {
  const value = stateSnapshot?.events?.[ROBOT_STORE_ASSESSMENT_EVENT_KEY];
...
#${OVERLAY_ID} .robot-store-assessment-name{color:#9ed6e8!important}
`;
  ...
  overlay.innerHTML = `
    <main class="main-screen kappa-jade-event-screen">
      <section class="visit-character-event kappa-jade-event" aria-live="polite">
        ...
        <button type="button" class="event-dialogue-card visit-event-dialogue glass-panel" data-robot-store-action="next">
          <small class="robot-store-assessment-name">ロボット</small>
          <strong>${esc(lines[dialogueIndex] || '')}</strong>
          <span>タップして進む</span>
        </button>
      </section>
    </main>
    <button type="button" class="event-safety-recovery" data-robot-store-action="end">イベント終了</button>
  `;
}
...
function replay(button) {
  ...
  queueMicrotask(() => {
    ...
    globalThis[OWN_RESUME_KEY] = button;
    try {
      button.click();
    } finally {
      if (globalThis[OWN_RESUME_KEY] === button) delete globalThis[OWN_RESUME_KEY];
...
function onClick(event) {
  const button = event.target instanceof Element ? event.target.closest('[data-robot-store-action]') : null;
  if (!button || !overlay?.contains(button)) return;
  const action = String(button.dataset.robotStoreAction || '');
  if (action === 'next') {
    advance();
    return;
  }
...
function startEvent(button, stateSnapshot, branchId) {
  ...
  const built = buildRobotStoreAssessmentDialogue(stateSnapshot, branchId);
  ...
  overlay.setAttribute('aria-label', 'ロボット・店舗品揃え評価イベント');
...
function intercept(button, action) {
  ...
  if (Math.random() >= ROBOT_STORE_ASSESSMENT_TRIGGER_CHANCE) return false;
...
globalThis[INTERCEPT_KEY] = (button, action) => {
  const ownResume = globalThis[OWN_RESUME_KEY] === button;
  if (ownResume) {
    bypass.delete(button);
    return false;
  }

  const anotherEventResume = globalThis[RESUME_KEY] === button;
  if (anotherEventResume) {
...
export const ROBOT_STORE_ASSESSMENT_EVENT_SPEC = Object.freeze({
  eventKey:ROBOT_STORE_ASSESSMENT_EVENT_KEY,
  triggerChance:ROBOT_STORE_ASSESSMENT_TRIGGER_CHANCE,
