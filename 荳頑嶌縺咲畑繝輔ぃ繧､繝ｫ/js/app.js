  ...
  root.innerHTML = `
    <main class="title-screen">
      <section class="title-panel glass-panel">
        <h1 class="sr-only">JEWELRY×JEWELRY</h1>
        ${!configured ? `
...
async function completeOnboarding() {
  ...
  try { await persist({ type: 'start', title: 'JEWELRY×JEWELRYを開始', details: `${name}としてゲームを開始しました。` }, before); renderMain(); } catch (_) {}
}
...
bootstrap().catch((error) => {
  console.error(error);
  root.innerHTML = `<main class="fatal-error"><h1>JEWELRY×JEWELRY</h1><p>起動できませんでした。</p><pre>${escapeHtml(error.message)}</pre></main>`;
});
