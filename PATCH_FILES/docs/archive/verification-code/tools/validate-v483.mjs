...
assert(VERSION === '0.10.483', `VERSIONが0.10.483ではありません: ${VERSION}`);
assert(DEFAULT_BIRTHDAY === '04-01', '誕生日の既定値が4月1日ではありません');
assert(sw.includes("const VERSION = '0.10.483'"), 'Service WorkerのVERSIONが古いです');
assert(indexHtml.includes('game.html?v=0.10.483'), 'index.htmlの読込番号が古いです');
assert(gameHtml.includes('js/app.js?v=0.10.483'), 'game.htmlの読込番号が古いです');
assert(app.includes("./daily-gems.js?v=0.10.483"), 'daily-gemsの読込番号が古いです');
...
// v0.10.483: 12月23日〜1月3日は体調不良イベントを発生させない。
{
  const context = { Date };
  vm.createContext(context);
...
console.log('v0.10.483 年末年始の体調不良停止期間検査: OK');
...
// v0.10.483: user-provided tool image implementation.
const toolImagesV483 = [
  ...
  'engraving-block.png','stamps.png','rotary-tool.png','ultrasonic-cleaner.png',
];
const gameDataTextV483 = fs.readFileSync(path.join(root, 'js/game-data.js'), 'utf8');
for (const filename of toolImagesV483) {
  ...
  assert(gameDataTextV483.includes(`./assets/images/tools/${filename}`), `データ参照がありません: ${filename}`);
}
assert(!fs.existsSync(path.join(root, 'assets/images/tools/buffer.png')), '未受領のbuffer.pngが勝手に追加されています');
console.log(`v0.10.483 validation passed: ${toolImagesV483.length} tool images implemented; buffer.png remains unreceived.`);
